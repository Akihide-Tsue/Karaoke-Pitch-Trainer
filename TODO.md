...
- [x] **4-1** 音程一致率: 歌唱終了時に computeScore で算出（**歌唱したところまで**の一致率とする）、「音程一致率 XX%」＋「保存する？」「保存しない」を表示
- [x] **4-2** 保存: 「保存する」なら録音 Blob とメタデータ（pitchData, score）を Dexie に保存。「今すぐ再生」を表示
- [x] **4-3** 再生画面: IndexedDB から直近 1 件を読み込み
- [x] **4-4** 再生画面（空）: 直近 1 件がないときは「まだ録音がありません」＋「練習する」で誘導
- [x] **4-5** 再生画面（あり）: 伴奏＋歌声を同時再生、**ガイドボーカル切替**ボタンでオケ／歌ありを切り替え、曲の音程バー＋歌唱の音程バー＋歌詞（同期）＋「練習に戻る」
